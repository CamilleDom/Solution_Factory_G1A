...
db.serialize(() => {
  // Créer les tables séparément
  db.run(`CREATE TABLE IF NOT EXISTS Users (
    username TEXT PRIMARY KEY NOT NULL,
    email TEXT UNIQUE NOT NULL,
    password TEXT NOT NULL,
    nb_pic INTEGER DEFAULT 0,
    role CHECK(role IN ('admin', 'user', 'anonyme')) NOT NULL,
    points REAL DEFAULT 0.0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
  );`);
  ...
  db.run(`CREATE TABLE IF NOT EXISTS Images (
    file_path TEXT PRIMARY KEY,
    label TEXT NOT NULL CHECK(label IN ('empty', 'full')),
    true_label TEXT NOT NULL CHECK(true_label IN ('empty', 'full')),
    upload_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    username TEXT NOT NULL
  );`);
  ...
  fs.createReadStream('./users.csv')
    ...
    .on('data', (row) => {
      insertUserStmt.run(
        row.username,
        row.email,
        row.password, // Si c’est un hash ou une chaîne
        parseInt(row.nb_pic),
        row.role,
        parseFloat(row.points)
      );
